// ==UserScript==
// @name         video-touch-enhancer-test
// @namespace    http://tampermonkey.net/
// @version      2.0.5
// @description  为所有网页视频播放器添加触屏手势（双击/长按/横滑/竖滑），并提供可视化设置面板
// @author       You
// @match        *://*/*
// @run-at       document-end
// @grant        unsafeWindow
...
(function() {
    ...
    // 是否处于顶层窗口。脚本会在所有 iframe 中运行（用于嵌入式播放器），
    // 但菜单命令与设置面板只应在顶层注册/弹出，否则在 YouTube 等多 iframe 站点上
    // 会重复注册「设置」，点击时可能触发隐藏子框架里的处理函数，导致面板建在不可见的子框架内。
    // 注意：这里用沙箱原生 window（而非 unsafeWindow），因为某些油猴版本的 unsafeWindow
    // 是代理对象，win.self 与 win.top 即便在顶层框架也可能不是同一引用，会误判。
    const isTopFrame = (() => {
        try { return window.self === window.top; } catch (err) { return false; }
    })();

    // Trusted Types 兼容：YouTube 等站点用 CSP 强制 require-trusted-types-for 'script'，
    // 此时一切「字符串转 DOM」的途径（innerHTML、DOMParser.parseFromString、
    // Range.createContextualFragment、<template>.innerHTML 等）都会被拦截并抛 TypeError。
    // 唯一合法途径是创建一个 Trusted Types policy，用它把字符串包装成 TrustedHTML 再赋给 innerHTML。
    // policy 必须从页面所在的 realm（unsafeWindow）创建，产出的 TrustedHTML 才会被页面的 innerHTML 接受。
    const ttPolicy = (() => {
        try {
    ...
    function setHTML(el, html) {
        ...
        try {
            ...
            console.warn("[touch-enhancer] setHTML 失败（Trusted Types 限制）：", err);
        }
    }
    ...
    const SETTINGS_KEY = "vte-settings-v1";
    ...
    // 仅在顶层窗口注册菜单命令并弹出设置面板，避免多 iframe 站点（如 YouTube）重复注册
    if (isTopFrame) {
    ...
    function showCtrl(controller) {
        ...
        const moveMouse = () => {
            ...
            sendMouseEvent(video, "mousemove", x, y);
            sendMouseEvent(video.parentElement, "mousemove", x, y);    
        };
    ...
    function hideCtrl(controller) {
        ...
        sendMouseEvent(video, "mouseleave", x, y);
        sendMouseEvent(video.parentElement, "mouseleave", x, y);
    ...
    function hideCtrlWhenMouseLeavesVideo(controller, e) {
        if (!controller.controlsVisible || controller.isDown) return;
        if (e && e.isTrusted === false) return;
        if (!isPointInVideoRect(controller, e.clientX, e.clientY)) hideCtrl(controller);
    }
    ...
    function createController(video) {
        ...
        controller.onDocumentMouseMove = (e) => hideCtrlWhenMouseLeavesVideo(controller, e);
        document.addEventListener("mousemove", controller.onDocumentMouseMove, true);

        // 遮罩层手势监听
        shield.addEventListener("pointerdown", (e) => { handleDown(controller, e); shield.setPointerCapture(e.pointerId); if (!controller.isLocked) setCtrl(controller, true); }, true);
        shield.addEventListener("pointermove", (e) => { handleMove(controller, e); if (!controller.isLocked) setCtrl(controller, true); }, true);
        shield.addEventListener("pointerup", (e) => { handleUp(controller, e); shield.releasePointerCapture(e.pointerId); }, true);
        shield.addEventListener("pointercancel", (e) => { handleUp(controller, e); shield.releasePointerCapture(e.pointerId); }, true);
    ...
    function scan() {
        ...
        document.querySelectorAll("video").forEach((video) => {
            ...
            try {
                controllers.set(video, createController(video));
            } catch (err) {
                console.warn("[touch-enhancer] 初始化视频失败：", err);
            }
        });
    ...
    // 初始化诊断日志：可在控制台确认脚本是否成功加载、是否顶层框架、视频是否被识别
    try {
        console.log("[touch-enhancer] v2.0.4 已初始化 | 顶层框架:", isTopFrame,
            "| TrustedTypes策略:", ttPolicy ? "已创建" : "无/未启用",
            "| 当前框架视频数:", document.querySelectorAll("video").length,
            "| 已绑定控制器:", controllers.size);
    } catch (err) {}
